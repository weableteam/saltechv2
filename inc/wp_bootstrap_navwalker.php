...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			$output .= '<ul role="menu" class="dropdown-menu shadow">';
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
			...
			} elseif ( 0 === strcasecmp( $item->title, 'divider' )) {
				$output .= '<li role="presentation" class="divider">';
			} elseif ( 0 === strcasecmp( $item->attr_title, 'dropdown-header' )) {
				...
				if ( ! empty( $item->attr_title ) ) {
					...
					if ( false !== $pos ) {
						...
						$item_output .= '<a' . $attributes . '><span class="bi-icon shadow-sm"><i class="bi ' . esc_attr( $item->attr_title ) . '" aria-hidden="true"></i></span>&nbsp;';
					}
				} else {
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$output .= '<ul' . ( $menu_id ? ' id="' . esc_attr( $menu_id ) . '"' : '' ) . ( $menu_class ? ' class="' . esc_attr( $menu_class ) . '"' : '' ) . '>';
					$output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html__( 'Add a menu', 'weable' ) . '</a></li>';
				$output .= '</ul>';
				if ( $container ) {
